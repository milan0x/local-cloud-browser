...
- [x] Body type + validity badges: section header shows live capsule badges — type pill (JSON blue, XML orange, Text gray) when body is non-empty, validity pill (Valid green checkmark, Invalid red xmark) for JSON/XML. "Read-Only" badge when helper is active (body TextEditor is disabled/dimmed).
...
- [x] First-use example seeding: `@AppStorage("hasUsedJsonHelper")` flag. First toggle auto-fills with `JSONHelperParser.defaultExample`. Subsequent toggles start empty — users already know the format.
...
- [x] Actions column: inline icon buttons matching S3 object browser pattern — View Details (eye), Copy Body (doc.on.doc), Delete (trash, red when enabled, gray when read-only). Uses `HStack(spacing: 8)` with `.buttonStyle(.borderless)` per button. Delete disabled in read-only mode.
