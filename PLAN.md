...
- [x] `CodeTextEditor` (`Modules/SQS/CodeTextEditor.swift`): `NSViewRepresentable` wrapping `NSTextView` in `NSScrollView` for the JSON helper input area. Replaces SwiftUI `TextEditor` to provide code-editor-like behavior:
  ...
  - **Auto-focus on appear:** `makeNSView` schedules `window?.makeFirstResponder(textView)` via `DispatchQueue.main.async`. When the JSON Helper is opened, focus moves from the message body TextEditor into the helper input automatically — prevents the cursor from blinking in the disabled message body.
...
- [x] Clickable toolbar region badge: for non-S3 modules (SQS, SNS, Secrets Manager), the region badge is a native `Menu` dropdown listing all regions. S3 shows a disabled dimmed `Menu` with "Global" label (not a plain HStack — must be a `Menu` so AppKit renders it as its own toolbar group, separate from the `.primaryAction` icon group). Selecting a region updates `appState.region` immediately. macOS menus support type-to-jump natively (start typing to highlight matching items). No explicit capsule background — AppKit's native toolbar group styling handles it. See "Known macOS SwiftUI Limitations" for why a popover with a text filter field was not used.
...
**Side effect — toolbar group capsules:** Adding `.toolbarHitTarget()` makes AppKit's group capsule (shared rounded background for same-placement items) more prominent. Non-control views (plain `HStack`) in a `ToolbarItem` get absorbed into adjacent groups. Fix: wrap non-interactive toolbar content in a `Menu` (even if disabled/empty) with `.menuStyle(.borderlessButton)` so AppKit treats it as its own distinct control with a separate group capsule. This is why the S3 "Global" badge uses a disabled `Menu` rather than a plain `HStack`.
