...
- [x] `DynamoDBTableAttributesView.swift`: 580pt sheet, compact card-style layout (ScrollView, not Form). Header: table name (`.title3`, bold) + colored status capsule (green ACTIVE / orange otherwise) + table ARN as `CopyableValue` (monospaced, caption). Stats row: 3 side-by-side cards (Items / Size / Billing) with `controlBackgroundColor` rounded rects. Key Schema: PK/SK rows with colored role badges (blue PK, purple SK) + attribute name + type capsule. Attributes: horizontal `WrappingHStack` flow layout of `name: Type` capsule pills. Indexes: cards with name + GSI/LSI type badge (orange/teal) + projection badge (blue) + inline key schema. Bottom bar: "Copy as AWS CLI" (describe-table) + "Done". Also accessible via inline `info.circle` button in item browser query controls header. `DynamoDBTableDetail.tableArn` parsed from `Table.TableArn`. Async loads via `service.describeTable()` with loading/error states.
- [x] `DynamoDBItemBrowserView.swift`: most complex view. **Mode selector**: Segmented Picker (Scan | Query). **Scan controls**: optional filter expression, Execute Scan button. **Query controls**: index picker (primary + GSIs/LSIs from table detail), partition key value field, sort key condition (operator picker: =, <, >, <=, >=, begins_with, between), optional filter expression, Execute Query button. **Results display**: `DynamoDBItemGrid` (AppKit NSTableView via NSViewRepresentable) — dynamic columns per attribute (PK first, SK second, rest alphabetical), colored type badges (S blue, N green, BOOL purple, etc.), inline cell editing for String/Number/Bool (double-click → edit, Enter=commit, Escape=revert, Tab=next cell), horizontal/vertical grid lines, 30px row height. Complex types (Map/List) open PutItem modal on double-click. Key columns not editable. Each cell edit saves immediately via `putItem()`. **Pagination**: status bar with "Items: X / Scanned: Y" + "Load More" button appending via `lastEvaluatedKey`. **Context menu**: NSMenu-based — View Details, Copy as JSON, Copy as AWS CLI (Get Item, Put Item), Edit, Delete. Multi-select: Delete N Items. Empty area: Put Item. **Auto-refresh**: Scan mode only, skipped during active editing.
- [x] `DynamoDBGridModels.swift`: `GridColumn` struct (name, isPartitionKey, isSortKey) + `computeGridColumns()` function — union of all attribute names across items, PK first, SK second, rest alphabetical.
- [x] `DynamoDBItemGrid.swift`: NSViewRepresentable wrapping `EditableTableView` (NSTableView subclass, `validateProposedFirstResponder` for cell editing) + `EditableTextField` (NSTextField subclass, double-click-to-edit lifecycle) + `Coordinator` (NSTableViewDelegate, NSTableViewDataSource, NSTextFieldDelegate, NSMenuDelegate). Callback-based API (`onCellEdit`, `onViewDetail`, `onDeleteItems`, etc.) — avoids SwiftUI update cycle issues. Selection sync via `isUpdating` guard. Column rebuild on attribute set change. Skips reload during active field editor.
...
## Phase 6c: Lambda Module
- [x] `LambdaModels.swift`: `LambdaFunction` (Identifiable, Hashable) — functionName, functionArn, runtime, handler, role, description, timeout, memorySize, codeSize, codeSha256, lastModified, state, version, environment dict, layers array. `init(from:)` parses PascalCase JSON (REST API). Computed: `isActive`, `formattedCodeSize`, `runtimeBadgeColor` (python=blue, nodejs=green, java=orange, dotnet=purple, ruby=red, custom=gray). CLI helpers: `getFunctionCLI()`, `invokeCLI()`, `listFunctionsCLI()`. `LambdaInvocationResult` — statusCode, payload, functionError, logResult with JSON detection/pretty-print.
- [x] `LambdaService.swift`: `@MainActor` ObservableObject wrapping `LocalStackClient` (implicitly unwrapped optional pattern). Methods: `listFunctions()` (paginated via `Marker`), `getFunction(name:)`, `createFunction(name:runtime:handler:role:zipData:description:timeout:memorySize:environment:)`, `updateFunctionConfiguration(...)`, `updateFunctionCode(name:zipData:)`, `deleteFunction(name:)`, `invokeFunction(name:payload:invocationType:)`.
- [x] `LocalStackClient.lambdaRequest(action:method:path:body:)`: REST API pattern — uses path-based routing with `/2015-03-31` prefix, Authorization header for region scoping. Read-only whitelist: ListFunctions, GetFunction, GetFunctionConfiguration, Invoke. Uses `skipReadOnlyCheck: true` since Lambda uses multiple HTTP methods for both reads and writes.
- [x] `Route.swift`: added `case lambda` with displayName "Lambda" and systemImage "function"
- [x] `ContentView.swift`: added `case .lambda: LambdaModuleView()` in `detailView(for:)`
- [x] `LastSessionStore.swift`: added `lambdaFunctionName: String?` to `LastSessionState`, `saveLambdaFunction(_:)` method, included in `clearSubResources()`
- [x] `LambdaToolbarState.swift`: ObservableObject bridge with pending action enum (viewDetails, invoke, createFunction, deleteSelected). `LambdaToolbar` ToolbarContent with Details (info.circle), Invoke (play), Create (+), Delete (trash) buttons, all with `.toolbarHitTarget()`. Disabled states: details/invoke disabled when no function selected, all mutating actions disabled in read-only mode.
- [x] `LambdaModuleView.swift`: HSplitView with function list (260pt) | detail pane or "Select a function" placeholder. `@StateObject` for service + toolbar state. Session restore via `LastSessionStore.load()` snapshot in `init()`. `LambdaModule` struct conforming to `LocalStackModule`.
- [x] `LambdaFunctionListView.swift`: 260pt function list with search, context menus, auto-refresh, double-click detector. Runtime color badges. Context menu: View Details, Invoke, Copy Name, Copy ARN, Copy as AWS CLI (Get Function, Invoke, List Functions), Create Function, Delete. Status bar with function count.
- [x] `LambdaFunctionDetailPaneView.swift`: right pane showing function overview — name, runtime badge, handler, state badge, configuration summary (timeout, memory, code size). Toolbar action handling for viewDetails and invoke.
- [x] `LambdaFunctionDetailView.swift`: 580pt detail sheet with Function Info (name, ARN, runtime, handler, role, state, version), Configuration (timeout, memory, code size, code SHA-256), Environment Variables section, Layers section. Edit button opens create/edit sheet.
- [x] `LambdaInvokeView.swift`: invocation sheet with CodeTextEditor for JSON payload, invocation type picker, execute button. Results: status code, response payload (JSON pretty-print), function error display, execution logs (base64-decoded).
- [x] `LambdaCreateFunctionView.swift`: create function sheet with name, runtime picker, handler, role fields, zip file upload via file picker (UniformTypeIdentifiers), description, timeout, memory size, environment variables editor. Collision detection against existing function names.
- [x] Read-only mode: create/delete/update disabled; invoke and browse allowed. Consistent with all other modules.
- [x] Session restore: `lambdaFunctionName` in `LastSessionState`, intra-session always on, cross-launch gated by `LastSessionStore.isEnabled`.

## Phase 6d: CloudWatch Logs Module
- [x] `CloudWatchLogsModels.swift`: `CloudWatchLogGroup` (Identifiable, Hashable) — logGroupName (id), arn, creationTime, retentionInDays, storedBytes. `init(from:)` parses camelCase JSON with ms-epoch timestamps (divide by 1000). Computed: `formattedStoredBytes`. CLI helpers: `describeLogGroupsCLI()`, `describeLogStreamsCLI()`. `CloudWatchLogStream` — logStreamName (id), arn, creationTime, firstEventTimestamp, lastEventTimestamp, storedBytes. CLI helper: `getLogEventsCLI()`. `CloudWatchLogEvent` — timestamp, message, ingestionTime. Computed: `isJSON`, `prettyPrinted`, `displayMessage`, composite `id` from timestamp+hash. `CloudWatchFilteredLogEvent` — adds logStreamName, eventId (used as id).
- [x] `CloudWatchLogsService.swift`: `@MainActor` ObservableObject wrapping `LocalStackClient`. Methods: `describeLogGroups()` (paginated via `nextToken`), `describeLogStreams(logGroupName:)` (paginated, ordered by LastEventTime descending), `getLogEvents(logGroupName:logStreamName:startFromHead:nextToken:limit:)` returns `LogEventsResult` (events + forward/backward tokens), `filterLogEvents(logGroupName:filterPattern:startTime:endTime:nextToken:limit:)` returns `FilteredLogEventsResult`, `createLogGroup(name:)`, `createLogStream(logGroupName:logStreamName:)`, `putLogEvents(logGroupName:logStreamName:message:)` (auto-generates ms-epoch timestamp), `deleteLogGroup(name:)`, `deleteLogStream(logGroupName:logStreamName:)`.
- [x] `LocalStackClient.cloudWatchLogsRequest(action:payload:)`: JSON protocol with `application/x-amz-json-1.1` content type and `X-Amz-Target: Logs_20140328.<Action>` header. Credential scope: `.../logs/aws4_request`. Read-only whitelist: DescribeLogGroups, DescribeLogStreams, GetLogEvents, FilterLogEvents. Uses `skipReadOnlyCheck: true`.
- [x] `Route.swift`: added `case cloudwatchLogs` with displayName "CloudWatch Logs" and systemImage "doc.text.magnifyingglass"
- [x] `ContentView.swift`: added `case .cloudwatchLogs: CloudWatchLogsModuleView()` in `detailView(for:)`
- [x] `LastSessionStore.swift`: added `cloudWatchLogsLogGroupName: String?` to `LastSessionState`, `saveCloudWatchLogsLogGroup(_:)` method, included in `clearSubResources()`
- [x] `CloudWatchLogsToolbarState.swift`: ObservableObject bridge with pending action enum (viewDetails, search, createLogGroup, deleteSelected). `CloudWatchLogsToolbar` ToolbarContent with Details (info.circle), Search (magnifyingglass), Create (+), Delete (trash) buttons, all with `.toolbarHitTarget()`. Search allowed in read-only mode. Create/delete disabled in read-only mode.
- [x] `CloudWatchLogsModuleView.swift`: HSplitView with log group list (260pt) | stream browser or "Select a log group" placeholder. `@StateObject` for service + toolbar state. Session restore via `LastSessionStore.load()` snapshot in `init()`. `CloudWatchLogsModule` struct conforming to `LocalStackModule`.
- [x] `CloudWatchLogsGroupListView.swift`: 260pt log group list with search, context menus, auto-refresh, double-click detector. Retention badge (days) and stored bytes in rows. Context menu: View Details, Copy Name, Copy ARN, Copy as AWS CLI (Describe Log Groups, Describe Log Streams), Create Log Group, Delete. Status bar with group count.
- [x] `CloudWatchLogsGroupDetailView.swift`: 580pt detail sheet, `.formStyle(.grouped)`. Log Group Info (name, ARN, retention days or "Never expire", stored bytes). Dates section (created). Footer: Done button.
- [x] `CloudWatchLogsCreateGroupView.swift`: create sheet (400pt wide, 180pt min height). Log group name TextField. Collision detection against existing group names. Create/Cancel buttons.
- [x] `CloudWatchLogsStreamBrowserView.swift`: right pane with S3-style drill-down via `@State activeStream`. **Stream list mode** (activeStream == nil): header with log group name + create stream button, SearchBarView when > 5 streams, list rows (name + last event time + stored bytes + chevron), click row sets activeStream. Context menu: View Events, Copy Name, Copy ARN, Copy as AWS CLI, Create Stream, Delete. Status bar with stream count. **Event viewer mode** (activeStream set): header with back button ("< Streams") + stream name + write event button, `CloudWatchLogsEventListView` below. Handles toolbar `.viewDetails` → detail sheet, `.search` → search sheet. Create stream sheet, delete stream alert. Auto-refresh on stream list (paused when drilled into events).
- [x] `CloudWatchLogsCreateStreamView.swift`: create stream sheet (400pt wide, 180pt min height). Log group name read-only label + log stream name TextField. Collision detection against existing stream names. Create/Cancel buttons.
- [x] `CloudWatchLogsEventListView.swift`: event list for a specific stream. Rows: timestamp (yyyy-MM-dd HH:mm:ss.SSS) + message (monospaced). JSON events get pretty-printed. Context menu: Copy Message, Copy Pretty-Printed (if JSON). Status bar: event count + "Load More" button (forward token pagination — stops when CloudWatch returns same token). `.task` loads on appear.
- [x] `CloudWatchLogsPutEventView.swift`: write event sheet (480pt wide, 380pt min height). Log group name + stream name (read-only labels). CodeTextEditor for message body. Write Event / Cancel buttons. Disabled in read-only mode.
- [x] `CloudWatchLogsSearchView.swift`: FilterLogEvents sheet (700pt wide, 600pt min height). Filter section: pattern TextField. Time range section: Toggle + DatePicker (from/to), defaults to last 1 hour. Search button. Results section: List of `CloudWatchFilteredLogEvent` — stream name badge (purple) + timestamp + message (monospaced). Context menu: Copy Message, Copy Pretty-Printed. Status bar: result count + "Load More" + Done button. Pagination via `nextToken`.
...
### DynamoDB Saved Queries
- **Quick-save query presets:** Save scan/query configurations (mode, partition key value, sort key condition, filter expression, selected index) as named presets per table. A dropdown/menu in the query controls header lets users switch between saved queries instantly. Stored in UserDefaults keyed by table name.
- **Recent queries:** Automatically track the last N (5-10) executed queries per table, shown in a recents menu. Lets users quickly re-run previous queries without re-entering parameters.
- **Export/import:** Allow exporting saved queries as JSON for sharing across teams or machines.

### DynamoDB Table Designer
- **Visual schema designer:** A dedicated view for designing new DynamoDB tables with a richer UI than the current create-table sheet. Would provide:
  - Visual partition key + sort key definition with drag-to-reorder
  - GSI/LSI designer: add/remove secondary indexes with their own key schema and projection type picker (ALL, KEYS_ONLY, INCLUDE with attribute selector)
  - Attribute definitions manager: add/remove typed attributes with validation
  - Capacity planning: toggle between on-demand and provisioned with RCU/WCU inputs
  - TTL configuration: enable/disable with attribute name picker
  - Tags editor: key-value pairs
  - Live preview of the `CreateTable` API payload (JSON) as you design
  - Schema validation: warns about common anti-patterns (e.g., missing sort key for query-heavy tables, over-provisioned GSIs)
- **Modify existing tables:** Extend the designer to support `UpdateTable` operations — add/remove GSIs, change billing mode, update provisioned throughput. Show current vs. proposed diff before applying.
