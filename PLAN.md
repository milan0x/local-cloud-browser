...
- [x] Persist region across launches: `appState.region` saved to UserDefaults via `didSet` (key `"region"`), restored on launch. Profile's region labeled "Default Region" in editor. On launch, `applyProfile()` runs first, then persisted region overrides it (first launch uses profile's default).
...
- [x] Default connection profile renamed from "Default" to "Default LocalStack Connection" — makes it clear this is a built-in default so users know to create their own profile if needed
- [x] Profile picker UI: replaced `Menu` (ellipsis.circle + dropdown chevron artifact) with a single edit button (ellipsis.circle, `.primary` style) per profile row. Delete moved into the connection editor form — red destructive "Delete Connection" button in its own section, with native `.alert()` confirmation showing profile name, endpoint, and region. Delete button only shown when editing an existing profile and more than one profile exists (cannot delete the last remaining profile). Editor frame height adjusts (440pt when delete is available, 380pt otherwise). On delete, auto-switches to the next available profile.
