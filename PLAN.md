...
- [x] Connection health check: polls `/_localstack/health` every 3 seconds with a 2-second manual Task-race timeout (ephemeral URLSession per check, invalidated after). Three-state `ConnectionStatus` enum: `.connected` (fast 200, < 1s, green `link` icon), `.unhealthy` (slow 200, orange `exclamationmark.triangle.fill` icon), `.disconnected` (timeout/error, gray `link` icon). Tooltip shows endpoint and status. Restarts automatically on profile switch. Replaces the old colored dot.
