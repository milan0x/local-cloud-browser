...
- [ ] Session restore: add `snsTopicArn: String?` to `LastSessionState`, `saveSNSTopic(_:)` to `LastSessionStore`, snapshot capture in `SNSModuleView.init()` gated by `LastSessionStore.isEnabled`, `restoreTopicArn` param on topic list view, restore logic after loading topics (see S3/SQS pattern)
...
- [ ] Delete secret
- [ ] Version history
- [ ] Session restore: add `secretName: String?` to `LastSessionState`, `saveSecretName(_:)` to `LastSessionStore`, snapshot capture in `SecretsManagerModuleView.init()` gated by `LastSessionStore.isEnabled`, `restoreSecretName` param on secrets list view, restore logic after loading secrets (see S3/SQS pattern)
...
- [x] Session restore — two-layer design:
  - **Intra-session memory (always on, not configurable):** Switching between services (S3 → SQS → S3) always restores the last selected bucket/queue/path. Saves happen unconditionally via `onChange` handlers. SwiftUI keeps module views alive in memory during route switches, so state persists naturally within a session.
  - **Cross-launch restore ("Open where I left off", configurable):** Gated by `LastSessionStore.isEnabled` (toggle in Settings > General, default `true`). When enabled, restores both the route (in `LocalStackNavigatorApp.init`) and per-module sub-resources (bucket/queue/path in each module's `init()`). When disabled, neither route nor sub-resources are restored on launch — modules start fresh. Each module view captures a snapshot from `LastSessionStore` in its `init()` via `@State` only when the flag is enabled, passes restore params to children (`restoreBucketName`, `restorePath`, `restoreQueueName`). Children use `@State hasRestoredSession/hasRestoredPath` flags to consume once per view lifecycle. Collision safety: `S3ObjectBrowserView` checks `restoreBucketName == bucket.name` before restoring path, preventing stale paths from bleeding into wrong buckets. Deleted resources: `first(where:)` returns nil → no selection, user sees the list.
