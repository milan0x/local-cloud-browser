...
- [x] Profile picker UI: replaced `Menu` (ellipsis.circle + dropdown chevron artifact) with explicit Edit (pencil) and Delete (trash) icon buttons per profile row. Delete shows native `.alert()` confirmation with profile name, endpoint, and region. Cannot delete last remaining profile (trash hidden when `profiles.count == 1`).
