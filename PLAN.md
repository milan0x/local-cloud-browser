...
- [x] Body type + validity badges: section header shows live capsule badges — type pill (JSON blue, XML orange, Text gray) when body is non-empty, validity pill (Valid green checkmark, Invalid red xmark) for JSON/XML. "Read-Only" badge when helper is active (body TextEditor is disabled/dimmed). Caption text below TextEditor explains: "Message body is generated by the JSON Helper below. Edit the helper to update."
- [x] `CodeTextEditor` (`Modules/SQS/CodeTextEditor.swift`): `NSViewRepresentable` wrapping `NSTextView` in `NSScrollView` for the JSON helper input area. Replaces SwiftUI `TextEditor` to provide code-editor-like behavior:
  ...
  - Monospaced font, undo support, vertical scrolling, width-tracking text container.
- [x] Auto-fill example on empty body: when JSON Helper is toggled on and the message body is empty, automatically fills with `JSONHelperParser.defaultExample`. Works every time (not just first use), so new messages always get a starting template. When body already has content, the helper opens empty so users can start fresh.
- [x] "Example Data" button (left side of helper button row):
  - **When helper text is empty:** directly fills with `JSONHelperParser.defaultExample`.
  - **When helper text has content:** toggles a syntax reference popover (read-only, no text fields — avoids NSPopover keyboard issues). Popover shows: syntax legend (key types: string, number, boolean, null, nested object, array), compact example, and a "Fill Example Data" button that replaces helper text and dismisses the popover. Monospaced font, 300pt width.
  - **Design decision — no bidirectional editing:** The upper message body TextEditor is disabled (read-only + dimmed) when the helper is active. Making it editable would require a reverse parser (JSON → DSL) which is complex and the round-trip isn't 1:1. One-way transformation (DSL → JSON) keeps the data flow predictable: helper text is the source of truth, JSON output is derived. Users who already have JSON prepared don't need the helper.
...
- [x] Message detail view: Queue Name added as first field (copyable via `CopyableValue`). Fields reordered by importance: Queue → Message ID → Type → Sent → Size → Group ID → First Received → Receive Count → MD5 (hash moved to bottom as least-used). Body pretty-print preserves original key order (no `.sortedKeys`) so fields appear as authored, not alphabetized.
