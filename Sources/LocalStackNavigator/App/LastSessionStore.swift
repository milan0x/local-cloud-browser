...
enum LastSessionStore {
    ...
    static func saveStepFunctions(tab: String?, stateMachineName: String?) {
        var state = load() ?? LastSessionState()
        state.stepFunctionsTab = tab
        state.stepFunctionsStateMachineName = stateMachineName
        save(state)
    }
    ...
    static func saveResourceGroup(_ name: String?) {
        var state = load() ?? LastSessionState()
        state.resourceGroupName = name
        save(state)
    }

    static func saveTranscribeJob(_ name: String?) {
        var state = load() ?? LastSessionState()
        state.transcribeJobName = name
        save(state)
    }
