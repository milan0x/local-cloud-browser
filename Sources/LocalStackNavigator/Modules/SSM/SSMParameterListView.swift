...
struct SSMParameterListView: View {
    ...
    var body: some View {
        ...
        .onChange(of: selectedParameterIDs) {
            if selectedParameterIDs.count == 1, let id = selectedParameterIDs.first {
                activeParameter = parameters.first { $0.id == id }
            } else {
                activeParameter = nil
            }
        }
    ...
    private var parameterListContent: some View {
        if isLoading && parameters.isEmpty {
            ...
            VStack(spacing: 0) {
                ...
                .background(SSMDoubleClickDetector {
...
private struct SSMDoubleClickDetector: NSViewRepresentable {
