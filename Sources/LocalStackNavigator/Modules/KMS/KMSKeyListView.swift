...
struct KMSKeyListView: View {
    ...
    var body: some View {
        ...
        .onChange(of: selectedKeyIDs) {
            if selectedKeyIDs.count == 1, let id = selectedKeyIDs.first {
                activeKey = keys.first { $0.id == id }
            } else {
                activeKey = nil
            }
        }
    ...
    private var keyListContent: some View {
        if isLoading && keys.isEmpty {
            ...
            VStack(spacing: 0) {
                ...
                .background(KMSDoubleClickDetector {
...
private struct KMSDoubleClickDetector: NSViewRepresentable {
