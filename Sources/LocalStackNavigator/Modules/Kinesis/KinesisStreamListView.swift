...
struct KinesisStreamListView: View {
    ...
    private var streamDeleteDisabled: Bool {
    ...
    private var streamListHeader: some View {
        HStack {
            ...
            Button {
                streamsToDelete = streams.filter { selectedStreamIDs.contains($0.id) }
            } label: {
                Image(systemName: "trash")
                    .foregroundStyle(streamDeleteDisabled ? .gray : .red)
            }
            .buttonStyle(.borderless)
            .disabled(streamDeleteDisabled)
            .help(selectedStreamIDs.count <= 1 ? "Delete Stream" : "Delete \(selectedStreamIDs.count) Streams")
        }
